...
std::map<A,B> morloc_packMap(std::vector<std::tuple<A,B>> items){
    std::map<A,B> m;
    for (const auto& t : items) {
        m[std::get<0>(t)] = std::get<1>(t);
    }
    return m;
}

template <class A, class B>
std::vector<std::tuple<A,B>> morloc_unpackMap(std::map<A,B> m){
    std::vector<std::tuple<A,B>> result;
